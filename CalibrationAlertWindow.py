...
class Ui_AlertWindow(object):
    ...
    def setupUi(self, AlertWindow, mainWindow, metric, range, value):
        ...
        self.stop_button.setGeometry(QtCore.QRect(40, 260, 141, 31))
        self.stop_button.setObjectName("stop_button")
        self.stop_button.clicked.connect(self.stopSystem)

        self.adjust_range_button = QtWidgets.QPushButton(AlertWindow)
        self.adjust_range_button.setGeometry(QtCore.QRect(200, 260, 160, 31))
        ...
        self.calibration_off_button.setGeometry(QtCore.QRect(380, 260, 141, 31))
        ...
        self.details_text.setGeometry(QtCore.QRect(20, 20, 500, 200))
        ...
        text = f"ALERT: {self.metric} readings are abnormal!\n\n{self.metric} was read as {self.value}, {difference:.2f} {unit} outside the range of {self.range[0]:.2f} to {self.range[1]:.2f}.\n\nHow would you like to resolve this issue?"
    ...
    def adjustRange(self):
        ...
        collected_value = self.value

        if (collected_value < self.range[0]):
            min_value = self.range[0] - (collected_value * 1.25)
        else:
            max_value = self.range[1] + (collected_value * 1.25)
